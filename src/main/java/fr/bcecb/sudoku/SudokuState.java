        ...
        for (int i = 0; i < 9; i++) {
            ...
            GuiElement candidateValueButton = new Button(++id, (width / 2) - (9 * btnSize / 2) + (btnSize * i) + 8, height - btnSize, btnSize, btnSize, true, String.valueOf(value), new ResourceHandle<>("textures/candidateValuesTextures.png") {}) {
              ...
              /*  @Override
                ...
                @Override
                public boolean isVisible() {
                    if (selectedX != -1 && selectedY != -1) return true;
               ...
               /* @Override
                ...
                public void onClick(MouseEvent.Click event) {
                    super.onClick(event);
                    if (sudoku.isCandidateValue(sudoku.computeCandidates(selectedX,selectedY),value)){
                        sudoku.getGrid()[selectedX][selectedY] = value;
                    }
                    else this.setTitleColor(Constants.COLOR_WHITE);
                    selectedX = -1;
                    selectedY = -1;
                }
            };
